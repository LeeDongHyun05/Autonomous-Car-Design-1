...
void loop() 
{
  ...
  if(flag[0]==0)
  {
    ...
    {
      motor_A_control(HIGH,220);                               //첫 코너 직진
  ...
  {
    wall_collision_avoid(215);
    if((front_sonar<=WALL_GAP_DISTANCE_HALF)&&(left_sonar<=WALL_GAP_DISTANCE)&&(left_sonar>=0))
    {
  ...
  {
    wall_collision_avoid(215);
    if((left_sonar>=0) && (left_sonar<=WALL_GAP_DISTANCE) && (front_sonar>=0) && (front_sonar<=WALL_GAP_DISTANCE_HALF))
    {
